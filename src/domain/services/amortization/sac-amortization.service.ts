...
export class SacAmortizationService {
  static generate(params: SacParams): Result<AmortizationSchedule, InvalidAmortizationParamsError> {
    ...
    // Absorb cumulative cent-level rounding drift into the final row's
    // principal so sum(principals) == originalPrincipal exactly at cent level.
...
function reconcileLastRow(installments: AmortizationInstallment[], originalPrincipal: Money): void {
  ...
  if (newPrincipalCents < 0n) return;
