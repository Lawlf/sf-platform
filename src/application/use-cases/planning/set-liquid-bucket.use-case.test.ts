...
function makeAsset(
  p: Partial<AssetEntity> & Pick<AssetEntity, "id" | "userId" | "profileId" | "category">,
): AssetEntity {
  return {
    label: "x",
...
function makeAssetsRepo(initial: AssetEntity[]): AssetRepositoryPort {
  return {
    ...
    findActiveByProfile: async () => {
      throw new Error("not used");
    },
    findActiveByProfileAndCategory: async () => {
      throw new Error("not used");
    },
    findByIdWithAllocations: async () => {
      throw new Error("not used");
    },
    findActiveWithAllocations: async () => {
      throw new Error("not used");
    },
    listStockTickersForProfile: async () => {
      throw new Error("not used");
    },
    listCryptoTickersForProfile: async () => {
      throw new Error("not used");
    },
    softDelete: async () => {
      throw new Error("not used");
...
describe("setLiquidBucket", () => {
  it("sets the bucket to a cash asset owned by the user", async () => {
    const res = await setLiquidBucket(
      { assets, settings },
      { userId: "u1", profileId: "profile-1", assetId: "buck" },
    );
  ...
  it("clears the bucket when assetId is null", async () => {
    const res = await setLiquidBucket(
      { assets, settings },
      { userId: "u1", profileId: "profile-1", assetId: null },
    );
    expect(res.ok).toBe(true);
    const stored = await settings.findByProfile("profile-1");
  ...
  it("rejects an asset that does not belong to the user", async () => {
    const res = await setLiquidBucket(
      { assets, settings },
      { userId: "intruder", profileId: "profile-intruder", assetId: "buck" },
    );
  ...
  it("rejects a non-cash asset", async () => {
    const res = await setLiquidBucket(
      { assets, settings },
      { userId: "u1", profileId: "profile-1", assetId: "car" },
    );
    expect(res.ok).toBe(false);
