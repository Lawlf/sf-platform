...
export function MobileTopBar({ displayName, notificationCount = 0 }: MobileTopBarProps) {
  ...
  return (
    <header
      ...
      className="fixed inset-x-0 top-0 z-30 flex items-center justify-between border-b border-[color:var(--border-soft)] bg-[color:var(--bg-app)]/85 px-3 py-2.5 backdrop-blur-md md:hidden"
    >
      <SimpleTooltip label="Ir para perfil" side="bottom">
        <Link
          href={"/app/perfil" as Route}
          aria-label="Ir para perfil"
          className="focus-ring flex items-center gap-2 rounded-full bg-[color:var(--color-brand-500)]/[0.12] py-1 pl-1 pr-3 text-[12px] font-semibold text-[color:var(--color-brand-800)] transition-colors hover:bg-[color:var(--color-brand-500)]/[0.2]"
        >
          <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[linear-gradient(135deg,#f28e25,#ef7a1a)] text-[11px] font-bold text-white shadow-[0_2px_8px_rgba(239,122,26,0.35)]">
            {initials}
          </span>
        ...
        <SimpleTooltip label="Notificações" side="bottom">
          ...
          >
            ...
            {hasNotifications ? (
              <span
                aria-hidden
                className="absolute -right-0.5 -top-0.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-[color:var(--semantic-negative)] px-1 text-[9px] font-bold text-white shadow-[0_0_0_2px_var(--bg-app)]"
              >
                {badgeLabel}
              </span>
            ) : null}
