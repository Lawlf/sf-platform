...
export function TransactionOverflowMenu({ transactionId, description, scheduled }: Props) {
  return (
    <Sheet>
      <SimpleTooltip label="Mais opções" side="bottom">
        <SheetTrigger aria-label={`Mais opções: ${description}`} className={TRIGGER_CLASS}>
      ...
      <SheetContent side="bottom" className="p-0">
        ...
        <div className="divide-y divide-[color:var(--border-soft)] border-t border-[color:var(--border-soft)]">
          <Link
            href={`/app/lancamentos/${transactionId}/editar` as Route}
          ...
          <DeleteTransactionButton transactionId={transactionId} scheduled={scheduled} />
        </div>
      </SheetContent>
    </Sheet>
  );
