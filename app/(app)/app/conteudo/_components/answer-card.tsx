...
export function AnswerCard({ num, title, description, onSelect, disabled, pending }: AnswerCardProps): ReactNode {
  return (
    ...
    >
      <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-[1.5px] border-[color:var(--border-soft)] text-[0.6875rem] font-extrabold tabular-nums text-[color:var(--text-secondary)] transition-colors group-hover:border-[color:var(--color-brand-500)]/35 group-hover:text-[color:var(--color-brand-800)]">
        {num}
      </span>
      <span className="min-w-0 flex-1">
        <span className="block text-[0.9375rem] font-bold leading-tight tracking-[-0.01em] text-[color:var(--text-primary)]">
          {title}
        </span>
        <span className="mt-1.5 block text-[0.78125rem] leading-[1.5] text-[color:var(--text-secondary)]">
          {description}
        </span>
      </span>
